...
interface ApproveRequestMobileSheetProps {
  request: AttendanceRequestWithRelations
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function ApproveRequestMobileSheet({
  request,
  open,
  onOpenChange,
}: ApproveRequestMobileSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
          ...
          <p className="text-sm text-muted-foreground">
            {preview ? (
              <>
                ...
                . They'll be notified in their portal.
              </>
            ) : (
        ...
        {preview && (
          <div className="mx-4 space-y-2 rounded-md border p-3 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Regular</span>
              <span className="font-mono font-medium">
                {formatDurationFromSeconds(preview.regularWorkedSeconds)}
              </span>
            </div>
            {preview.overtimeSeconds > 0 && (
              <div className="flex items-center justify-between">
